...
constexpr long long int sample_rate = 20ll * 1000000ll;
...
{
    const uint64_t max_phase = uint64_t(sample_rate * 10);
    const double freq_scale = 0.1 / (sample_rate / 32);
...
private:
    ...
    NACS_NOINLINE __attribute__((target("avx512f,avx512bw"), flatten)) void generate()
    {
        ...
        // const uint64_t freq_cnt = uint64_t(1.953125e8);
        // // const uint64_t freq_cnt = 1562500000ull; // In unit of 0.1Hz
        std::vector<int64_t> phase_cnt(nchn,sample_rate * 10 * 0.25);
        while (true) {
            //Log::log("1");
            size_t sz;
            auto ptr = get_write_ptr(&sz); // int16_t* pointer
