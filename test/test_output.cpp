...
static __m512 xsinpif_pi(__m512 d)
{
...
static __m512i calc_sins2(float phase, float freq, float amp)
{
...
int main()
{
    ...
    hdl.set_param(SPC_AMP0, 160); // Amp
    hdl.set_param(SPC_FILTER0, 0);

    Stream stream(0.9f, 51.46e6);

    size_t buff_sz;
    auto buff_ptr = stream.get_read_buff(&buff_sz);
    hdl.def_transfer(SPCM_BUF_DATA, SPCM_DIR_PCTOCARD, 4096 * 4,
