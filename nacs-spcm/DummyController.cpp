...
namespace Spcm {
// TIME TRACKING STUFF
void DummyController::tryUnlock(){
...
inline bool DummyController::checkRequest()
{
...
NACS_EXPORT() void DummyController::startWorker()
{
    if (workerRunning())
        return;
    {
        ...
        m_worker = std::thread(&DummyController::workerFunc, this);
    }
}
NACS_EXPORT() void DummyController::stopWorker()
{
    if (!workerRunning())
        return;
...
NACS_EXPORT() void DummyController::runSeq(uint32_t idx, Cmd *p, size_t sz, bool wait){
    uint32_t nwrote;
    do {
...
// //hdl.check_error();
    printf("Controller initializing\n");
    m_initialized = true;
}

void DummyController::initChnsAndBuffer()
{
    printf("Controller initializing channels and buffers\n");
...
NACS_EXPORT() void DummyController::force_trigger()
{
...
inline void DummyController::startDMA(uint64_t sz)
{
    if (DMA_started)
    {
        return;
...
void DummyController::workerFunc()
{
    // set_fifo_sched();

    std::lock_guard<std::mutex> locker(m_worker_lock);
    printf("Controller worker starts\n");
    // TIMING STUFF
