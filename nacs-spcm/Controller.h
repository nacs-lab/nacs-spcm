                ...
                m_conf(conf)
          {
              ...
              if (phys_chn != 1 && phys_chn != 2)
              {
                  ...
                  for (int i = 0; i < n_card_chn; i++) {
                      m_stm_mngrs.emplace_back(new StreamManager(*this, m_conf, n_streams[i], 2, 1, amp_scales[i], cmd_underflow, cmd_underflow, false));  // hard coded 2 channels per stream
...
// for non buffer overrun, for now, do not handle and just throw
      ...
      private:
          ...
          uint64_t buff_sz_nele{8 * 1024ll * 1024ll / 2}; // 8 // 2/2 //4 factor of 4 1 channel output latency of 6.71 ms. Software buffer size
          uint64_t hw_buff_sz_nele{2 * 1024ll * 1024ll};  // 2// 1 //2 1 channel output latency of 1.67 ms. Hardware buffer size number of elements
