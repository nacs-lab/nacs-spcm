...
public:
    ...
    inline std::pair<uint32_t,uint32_t> delChn(uint32_t chnid) {
        ...
        uint32_t idx = getChn(chnid);
        if (idx == chn_map.size()){
            return std::make_pair(idx,UINT32_MAX); // entry doesnt exist
        }
        else {
            ...
            chn_map[stream_last] = UINT32_MAX;
            tot_chns--;
            return stream_info;
        }
    }
...
public:
    uint64_t getControllerOutputCnt();
    inline void reset()
    {
...
protected:
          ...
          m_output((int16_t*)mapAnonPage(output_buf_sz, Prot::RW), output_buf_sz / 2, output_buf_sz / 2)
    {
        ...
        for (int i = 0; i < n_streams; i++) {
            ...
            stream_ptr = new Stream<128>(*this, m_conf, step_t, amp_scale, cmd_underflow, underflow, i, start);
...
private:
    ...
    uint64_t output_buf_sz = 256 * 1024ll * 1024ll; // in bytes. Let Streams below it throttle the filling of this buffer.
    uint64_t wait_buf_sz = 32 * 1024ll * 1024ll;
