...
inline void StreamManagerBase::actual_send_cmds(uint32_t stream_idx, Cmd *cmd, size_t sz, bool isAnalog)
{
...
inline void StreamManagerBase::send_cmds(Cmd *cmd, size_t sz, bool isAnalog)
{
    ...
    // after exiting loop, may still need to distribute some commands.
        ...
        for (int i = 0; i < sz; ++i){
            //printf("cmd chn: %u\n", cmd[i].chn);
            std::pair<uint32_t, uint32_t> this_stream_info;
            if (!isAnalog) {
                this_stream_info = chn_map.ChnToStream(cmd[i].chn);
            }
            else {
                this_stream_info = analog_chn_map.ChnToStream(cmd[i].chn);
            }
...
NACS_EXPORT() void StreamManagerBase::distribute_cmds()
{
    ...
    std::vector<Cmd> analog_non_const_cmds; // non constant commands
    analog_non_const_cmds.reserve(1024);
    Cmd *analog_first_cmd = nullptr;// first_cmd is first cmd in a group to send
    int64_t t = 0;
    size_t sz_to_send = 0;
    size_t analog_sz_to_send = 0;
    while ((cmd = get_cmd())){
        // std::cout << "Considering " << *cmd << std::endl;
        if (cmd->op() == CmdType::Meta) {
            // send out previous commands and reset first_cmd
            send_cmds(first_cmd, sz_to_send, false);
            send_cmds(analog_first_cmd, analog_sz_to_send, true);
            first_cmd = nullptr;
            sz_to_send = 0;
            analog_first_cmd = nullptr;
            analog_sz_to_send = 0;
            ...
            switch (cmd-> op()) {
                ...
                case CmdType::AnalogFn:
                case CmdType::AnalogVecFn:
                    analog_non_const_cmds.push_back(var_cmd);
                    break;
            }
            //if (!first_cmd){
            ...
            if (cmd->t != t) {
                ...
                first_cmd = non_const_cmds.data() + non_const_cmds.size() - 1;
                analog_sz_to_send = 1;
                analog_first_cmd = analog_non_const_cmds.data() + analog_non_const_cmds.size() - 1;
